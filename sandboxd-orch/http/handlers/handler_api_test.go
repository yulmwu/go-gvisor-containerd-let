...
func TestHandlers_AllEndpoints(t *testing.T) {
	...
	must(http.MethodPost, "/externals", []byte(`{"id":"ext-1","spec":{"nodeId":"n1","external":"host1.swua.kr"}}`), 200)
	must(http.MethodPost, "/externals", []byte(`{"id":"ext-2","spec":{"nodeId":"missing","external":"host1.swua.kr"}}`), 400)
